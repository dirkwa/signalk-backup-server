...
- [src/services/kopia-client.ts](src/services/kopia-client.ts) — typed wrapper over the kopia CLI. Full-snapshot methods (`createSnapshot`, `listSnapshots`, `restoreSnapshot` with progress callbacks, `connect`/`disconnect` for cloud mode) plus sub-path methods used by selective restore (`listSnapshotEntries`, `restoreSubtree`, `showFileByObjectId`). `KopiaEntryNotFoundError` carries the "snapshot or sub-path doesn't exist" case so route handlers can return 404 instead of leaking a generic 500. All commands go through `runKopia` which knows about timeouts and "we're connected to a cloud repo, every command starts a fresh rclone subprocess" (45s timeout for cloud commands vs. 600s default).
- [src/services/backup-machine.ts](src/services/backup-machine.ts) — XState machine for full restore-with-rollback. States: `idle → preparing → extracting → installing → restarting → verifying → completed | failed → rolling_back → rolled_back`. Every restore creates a "safety backup" before clobbering anything.
- [src/services/restore-service.ts](src/services/restore-service.ts) — drives the full-restore machine. Takes a snapshot id, validates, runs the safety backup, restores files, restarts SignalK (via signalk-container if available), health-checks, rolls back on any failure.
- [src/services/restore-partial-machine.ts](src/services/restore-partial-machine.ts) — sibling XState machine for selective (file/sub-dir) restore. Simpler states: `idle → preparing → safety_snapshotting → extracting → verifying → completed | failed → rolling_back → rolled_back`. No `installing` / `restarting` because a single-file copy doesn't justify restarting SignalK.
- [src/services/restore-partial-service.ts](src/services/restore-partial-service.ts) — drives the partial machine. `resolvePartialTarget` is the security boundary — realpath-resolves the target against `signalkDataPath`, rejects `..` segments / NUL bytes / paths outside the root. Reject-list (`package.json`, `node_modules/`, `.kopia*`) only fires for `targetMode: 'original'`; custom-mode is a copy the user inspects, so the side-effect concerns don't apply. The "safety backup" is a sibling rename (atomic, instant, reversible) rather than a Kopia snapshot.
- [src/services/restore-lock.ts](src/services/restore-lock.ts) — shared probe registry. Full and partial services register their `isRestoring()` callbacks; both consult `isAnyRestoreActive()` before starting. Avoids a circular import while keeping the cross-service lock semantics explicit.
...
- [src/api/backup-routes.ts](src/api/backup-routes.ts) — the largest router. All `/api/backups/*` endpoints (list, create, restore, upload, download, plus the selective-restore trio `/tree`, `/download-subtree`, `/restore-partial`). Routes register via `createApiRouter` which auto-populates the OpenAPI doc — no separate registry file to update. **Route order matters**: specific paths (e.g. `/restore-partial/status`, `/scheduler`) must precede parameterised paths like `/:id` or they'll be shadowed.
- [src/middleware/validate.ts](src/middleware/validate.ts) — TypeBox request validation. Express 5 made `req.query` getter-only, so the middleware writes validated/defaulted query values back via `Object.defineProperty` rather than plain assignment.
...
- **Full and partial restores share one lock.** Both `restoreService.isRestoring()` and `restorePartialService.isRestoring()` register their callbacks in `restore-lock.ts`. Every entry point (full restore, partial restore, partial host-restore on the plugin side) consults `isAnyRestoreActive()` before starting. Don't add a third state machine without registering it too — concurrent restores corrupt the live tree.
- **Partial-restore reject-list only fires for original-mode.** `package.json`, `node_modules/`, `.kopia*` would silently break the running server if restored in-place (npm install needed, repo state, etc.). Custom-mode is just a copy the user inspects, so the reject-list is bypassed. Both branches go through the same path-safety realpath check, which still enforces "must resolve under signalkDataPath".
- **`/restore-partial` writes inside the container only.** The route restores under `signalkDataPath`. To land bytes at host-only paths like `/tmp` or `/media/usb/...` the user goes through the plugin's host-write route, which streams from `/download-subtree` and writes locally. Don't try to add host paths to this server-side route — the container can't see them.
- **`req.query` is getter-only in Express 5.** Plain assignment after TypeBox defaulting throws "Cannot set property query". The middleware uses `Object.defineProperty` to write the defaulted object back. If you add a new route with `query:` validation, this is the path that runs — don't try to bypass it.
- **Trailing slash on `customPath` means "into this directory".** `resolvePartialTarget` appends the source basename when `customPath` ends with `/`, so `tmp/` + file `package.json` lands at `tmp/package.json` rather than overwriting `tmp` as a file. The plugin's host-restore writer applies the same rule; if you change one, change the other.
...
- **Update a branch by rebasing onto `main`, never by merging `main` into it.** Push the result with `git push --force-with-lease --force-if-includes origin HEAD`, naming the remote that holds the branch (a fork's, not this repository, for a contributor). A merge commit from `main` is not a conventional commit and buries the change under unrelated history — on a fork's pull request as much as on our own branches.
